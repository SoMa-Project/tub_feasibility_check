                          ...
                          const std::string& name)
{
  ...
  auto body = bullet_scene.getModel(ifco_model_index)->create();
  ...
  if (viewer)
  {
    auto viewer_body = viewer->scene_graph->getModel(ifco_model_index)->create();
    viewer_body->setName(name);
    auto viewer_sg_shape = viewer_body->create(vrml_shape);
    viewer_sg_shape->setTransform(box_pose);
    viewer_body->add(viewer_sg_shape);
  }
}
...
IfcoScene::plan(const rl::math::Vector& initial_configuration, const rl::math::Transform& goal_pose,
